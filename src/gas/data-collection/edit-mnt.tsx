...
import { MAINTENANCE_TYPES, maintenanceDB, MaintenanceReading } from "./types";
...
export default function App(props: Props) {
  ...
  const fetchData = async () => {
    ...
    const snapshot = await get(child(maintenanceDB, `/${props.matches.id}`));

    if (!snapshot.exists()) {
      return;
    }

    const val: MaintenanceReading = snapshot.val();
    reset({
      date: val.date,
  ...
  const handleSubmitForm: SubmitHandler<MntForm> = async (data) => {
    ...
    try {
      ...
      await update(child(maintenanceDB, props.matches.id), {
