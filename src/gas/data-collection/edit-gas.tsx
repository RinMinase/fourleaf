...
export default function App(props: Props) {
  ...
  const fetchData = async () => {
    ...
    const snapshot = await get(child(gasDB, `/${props.matches.id}`));

    if (!snapshot.exists()) {
      return;
    }

    const val: GasReading = snapshot.val();
    reset({
      date: val.date,
      from_bars: val.from_bars,
      to_bars: val.to_bars,
      odometer: val.odometer,
      price_per_liter: val.price_per_liter,
      liters_filled: val.liters_filled,
    });
  ...
  const handleSubmitForm: SubmitHandler<Form> = async (data) => {
    ...
    try {
      ...
      await update(child(gasDB, props.matches.id), {
